...
├── model/       Kayit — bir kaydın kendisi, tarih/saat/sayı çözümlemesi
├── store/       Json, KayitDeposu, DisaAktarim — okuma, yazma, dışa aktarım
├── ui/          Tema, Ikonlar, AnaPencere ve üç panel
...
`ui` paketi `store` ve `model`'i tanıyor, ters yönde tek bir import yok. Veri
katmanında `javax.swing` de geçmiyor; bu yüzden depo ve JSON çözümleyici pencere
açmadan sınanabiliyor.

Tek gerçek kaynak `KayitDeposu`'nun bellekte tuttuğu liste. Tablo modeli bunun
kopyasını çıkarmıyor, aynı `Kayit` nesnelerine referans veriyor — bir hücre
düzenlendiğinde doğrudan kayıt değişiyor. Diske yazma 600 ms geciktiriliyor, böylece
art arda gelen tuş vuruşları tek yazmada toplanıyor.
